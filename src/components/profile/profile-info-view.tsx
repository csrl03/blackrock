...
export default function ProfileInfo() {
  ...
  const [profileImage, setProfileImage] = useState<string>(authorData?.avatar?.thumbnail || AuthorImage);
  const [userId, setUserId] = useState<number>(authorData?.id || 1);

  useEffect(() => {
    // Si tienes userData en localStorage, úsalo como fuente prioritaria
